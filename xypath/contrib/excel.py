...
def excel_range(address):
    ...
    return (
        excel_address_coordinate(address1, partial=True),
        excel_address_coordinate(address2, partial=True),
    )


def excel_address_coordinate(address, partial=False):
    """Given a cell reference, return a tuple for inserting into ``Table.get_at()``."""
    ...
    if (row is None or col is None) and not partial:
        raise InvalidExcelReference(f"{address!r} is partial")
    return (col, row)
...
def filter_one(self, filter_by):
    ...
    if filter_length == 0:
        ...
        foundmsg = f"{filter_length}: {filtered.excel_locations()}"

    return filtered.assert_one(
        errormsg.format(xypath.describe_filter_method(filter_by), foundmsg)
    )
